...
async function predictionChatGPT(id) {
  try {
    ...
    const descriptionReq = await callChatGpt(
      ...
      Task 1 (Logic description generation): I give you the textual description of a bpmn process, can you give me back the description of the working of this process, explicit the protagonists if there are messages exchaged between more participants? Just a brief description of at most 30 lines:
      ${currentXML}
      ...
      Go to head and you have to output this information following this format, taking as example the process of a makeup store that needs to register a new customer in its database: 'Data Controller: Makeup Store; Data Subject: Customer'
      ...
      switch (id) {
        case "dropDownA":
          const hasPersonalDataReq = await callChatGpt(
            `Consider to be a GDPR expert that needs to analyze a process in order to understand if it handles personal data of the Data subject.
            Task 1: Check this logical description of the process:
            ${description}
            to understand who is the Data Subject and who is the Data Controller if they are present in the process. If they are not present in the process, check if there is some activity with a name that suggests the handle of personal data. You don't have to invent nothing, just use the material I provide to you.
            Task 2: Retrieve the definition of personal data and find out if in the process handled personal data of the Data Subject.
            Double-check your answer by analyzing the textual description of the process and searching for activities that suggest the handle of personal data of the data subject:
            ${currentXML}
            ...
            the roles definition: Data Controller: Makeup store; Data Subject: Customer
        ...
        case "dropDownC":
          const hasRightToAccessReq = await callChatGpt(
            ...
            If there aren’t two different participants, but ONLY IN THIS CASE, if you find two participants that can act Data Subject and data Controller, you have to find the Message Flow between them to consider a request of accessing data as it is, check if still exists this behavior without the two partecipation, taking in consideration the previous case, it could be:
            {Participant: Phone Company  linked to:  linked to: [Start Exclusive Gateway Gateway_0iyfbzs (only one of the path can be taken) condition to check in order to proceed with the right path ‘is valid?’ different paths: Path of Gateway_0iyfbzs taken if: ‘yes’ linked to  linked to Gateway_0lujbik Path of Gateway_0iyfbzs taken if: ‘no’ linked to Gateway_0lujbik End paths of ExclusiveGateway Gateway_0iyfbzs ] [Closure Exclusive Gateway Gateway_0lujbik] linked to:  End Participant: Phone Company}.
        ...
        case "dropDownD":
          const hasRightOfPortabilityReq = await callChatGpt(
            `Consider to be a GDPR specialist that needs to understand the level of GDPR compliance of a process. The logical description of the process is this one:
              ...
              This is the provided text description of the process:
              ${currentXML}.
              Objective: You have to understand if the process provided is compliant with the right of portability or not. By definition the Right of Portability is defined as follows: At any moment, the Data Subject (the person the data is about) can request the portability of the data associated with her to third parties, and the Data Controller (the entity that collects the data of the subject) has the obligation to satisfy this request.
              ...
              In order to achieve this objective, follows these steps:
              Step 1: Identify if there is an activity where the Data Subject requests the portability of her personal data to the Data Controller. The request must be initiated by the Data Subject and must arrive at the Data Controller check the exchanges of messages indicated in the textual description.
        ...
        case "dropDownE":
          const hasRightToRectifyReq = await callChatGpt(
            ...
            An example could be:
            { Participant: Participant_0t4txcj (empty pool) } {Participant: Participant_0wfkdnl <Activity name: Request of data rectification Type: StartEvent ID: Event_0c4vuea Exchange with other partecipations: Receive a message from bpmn :ParticipantParticipant_0t4txcj /> linked to:  linked to:  End Participant: Participant_0wfkdnl}.
            ...
            Step 2: If you do not find such an activity or if the process does not contain more participants, check if in the textual description there exists an event in which a request for rectification arrives. If so, print ‘Yes’; Or check if exists some kind of ‘Right of rectification handler’. If exists something that by the name suggested some kind of handling of this right, please print ‘Yes’.
            The request of rectification must be satisfied. The case in which is denied, like this:
            { Participant: Participant_0t4txcj (empty pool) } {Participant: Participant_0wfkdnl  linked to:  linked to:  End Participant: Participant_0wfkdnl} must not be taken in consideration, in this case the answer is ‘No’.
        ...
        case "dropDownH":
          const hasRightToRestrictProcessingReq = await callChatGpt(
            ...
            Step 1: If there are more participants, identify if there is an activity where the Data Subject communicates her will to restrict the processing of their data. Also, check if the data controller acknowledges the request of the data subject and restricts the processing of the data accordingly.
